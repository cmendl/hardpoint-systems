...
///
currsum_even_odd_t SweepTable(field_t *f, event_table_t *tab)
{
	// time period
	...
	for (islot = 0; islot < NUM_SLOTS; islot++)
	{
		...
		while (slot->num > 0)	// as long as there are active events in current time slot
		{
			...
			// accumulate current
			if (ev->iprt % 2 == 0)	// even lattice site
			{
				J.evn.r += J_cur.r;
				J.odd.r += J_cur.r_prev + J_cur.r_next;
				J.odd.p += J_cur.p_next;
				J.odd.e += J_cur.e_next;
			}
			else		// odd lattice site
			{
				J.odd.r += J_cur.r;
				J.evn.r += J_cur.r_prev + J_cur.r_next;
				J.evn.p += J_cur.p_next;
				J.evn.e += J_cur.e_next;
			}
	...
	const double scale = 1.0 / (NUM_SITES * period);
	J.evn.r *= scale;
	J.evn.p *= scale;
	J.evn.e *= scale;
	J.odd.r *= scale;
