...
int main()
{
	...
	const double energy = AverageEnergy(beta);
	...
	for (is = 0; is < nsamples; is++)
	{
		...
		for (ia = 0; ia < 2; ia++)
		{
			for (ip = ia; ip < NUM_SITES; ip += 2)
			{
				const lattpoint_t *point = &f.pts[ip];
				pointdata[ip].r = point->r;								// elongation
				pointdata[ip].p = mass[ia] * point->v;					// momentum
				pointdata[ip].e = LattPoint_Energy(mass[ia], point);	// energy
			}
		}
