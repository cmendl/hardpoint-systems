	...
	for (ic = 0; ic < ncycles; ic++)
	{
		...
		for (is = 0; is < nsamples; is++)
		{
			...
			for (it = 0; it < 2*n_max_corr; it++)	// twice the correlation time interval to collect all time differences equally often
			{
				...
				Jt[it].r += J_add.r / period;
				Jt[it].p += J_add.p / period;
				Jt[it].e += J_add.e / period;
		...
		sprintf(path, "../output/current/sim_%i/sim_%i_ns%i_Javr_%04d.dat", itime, itime, nsamples, ic); WriteData(path, J_tcorr.avr, sizeof(current_t), 2*n_max_corr, false);
		sprintf(path, "../output/current/sim_%i/sim_%i_ns%i_Jsqr_%04d.dat", itime, itime, nsamples, ic); WriteData(path, J_tcorr.sqr, sizeof(cov_mat_t),   n_max_corr, false);

		sprintf(path, "../output/current/sim_%i/sim_%i_ns%i_dt%d_Javr_%04d.dat", itime, itime, nsamples, NUM_AVR_CURRENT, ic); WriteData(path, J_tcorr_longdt.avr, sizeof(current_t), 2*J_tcorr_longdt.num_t, false);
		sprintf(path, "../output/current/sim_%i/sim_%i_ns%i_dt%d_Jsqr_%04d.dat", itime, itime, nsamples, NUM_AVR_CURRENT, ic); WriteData(path, J_tcorr_longdt.sqr, sizeof(cov_mat_t),   J_tcorr_longdt.num_t, false);

		printf("saving integrated current to disk...\n");
		sprintf(path, "../output/current/sim_%i/sim_%i_ns%i_Jint_%04d.dat", itime, itime, nsamples, ic); WriteData(path, J_int, sizeof(current_t), 2*nsamples, false);
