...
/// and estimating the distribution of time-integrated currents minus integrals over field variables with length c*t.
//
...
int main(int argc, const char* argv[])
{
	...
	const double c_sound = 1.74264199616297767;		// speed of sound for these concrete pressure and beta values

	const double ell_avr = AverageElongation(pressure, beta);
	const double  en_avr = AverageEnergy(pressure, beta);

	const double matR_0r =  2.103076069630312663;	// the (0,r) entry of the R matrix (normal mode 0 and physical coordinate elongation 'r')
	const double matR_1r = -0.806691797719916615;	// the (1,r) entry of the R matrix (normal mode 1 and physical coordinate elongation 'r'), same as (-1,r) entry
	const double matR_0e =  1.752563391358593886;	// the (0,e) entry of the R matrix (normal mode 0 and physical coordinate energy 'e')
	const double matR_1e =  0.779958498702550879;	// the (1,e) entry of the R matrix (normal mode 1 and physical coordinate energy 'e'), same as (-1,e) entry
	...
	for (ic = 0; ic < ncycles; ic++)
	{
		...
		for (is = 0; is < nsamples; is++)
		{
			...
			for (ip = 0; ip < cti; ip++)
			{
				...
				F_int[0].p +=                   f.pts[(0 - ip) & SITE_MASK].p;
				F_int[0].e += LattPoint_Energy(&f.pts[(0 - ip) & SITE_MASK])  -  en_avr;
			}
			for (ip = 1; ip < NUM_SITES; ip++)
			{
				F_int[ip].r = F_int[ip - 1].r +                   f.pts[ip].r -                   f.pts[(ip - cti) & SITE_MASK].r;
				F_int[ip].p = F_int[ip - 1].p +                   f.pts[ip].p -                   f.pts[(ip - cti) & SITE_MASK].p;
				F_int[ip].e = F_int[ip - 1].e + LattPoint_Energy(&f.pts[ip])  - LattPoint_Energy(&f.pts[(ip - cti) & SITE_MASK]);
			}
			...
			// complete momentum integration for elongation current; 'period' has already been subtracted
			for (ip = 0; ip < NUM_SITES; ip++)
			{
				lattpoint_t *point = &f.pts[ip];
				J_int[ip].r -= (0 - point->t) * point->p;
			}
			...
			for (ip = 0; ip < NUM_SITES; ip++)
			{
				...
				J_inr[ip].m[0] = matR_1r * J_int[ip].r - J_int[ip].p + matR_1e * J_int[ip].e;	// mode -1
				J_inr[ip].m[1] = matR_0r * J_int[ip].r               + matR_0e * J_int[ip].e;	// mode  0
				J_inr[ip].m[2] = matR_1r * J_int[ip].r + J_int[ip].p + matR_1e * J_int[ip].e;	// mode  1
			}
			...
			for (ip = 0; ip < NUM_SITES; ip++)
			{
				...
				double Fm1 = matR_1r * F_int[iq].r - F_int[iq].p + matR_1e * F_int[iq].e;
				double F_1 = matR_1r * F_int[ip].r + F_int[ip].p + matR_1e * F_int[ip].e;
